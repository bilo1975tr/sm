...
- **Tek Kopyada Çalışma (Single Instance / Mutex):** Uygulama sistem tepsisine atıldığında, kullanıcı tekrar kısayoldan açmaya çalışırsa 2. bir pencere açılması KESİNLİKLE engellenecektir. Çoklu örnek (Double Instance) önlemi için `App.xaml.cs` katmanında her zaman Global `Mutex` yapısı var olacaktır.
- **Sahte İstatistik ve Simülasyon Yasağı (No Mock/Fake Data):** Uygulama içerisinde rastgele sayı üreterek "Canlı İzleyici (Viewer)" veya oylama simüle etmek KESİNLİKLE YASAKTIR. Veriler tamamen organik ve gerçek sunucuya veya gerçek kullanıcı etkileşimine dayalı olmalıdır.
- **Web Player Proxy Senkronizasyonu:** Yerel ağ sunucusu (HTML/M3U) sadece listeyi sunmakla kalmayacak; oynatıcıda (`PlayerView.xaml.cs`) yapılan tüm AceStream, YouTube adaptasyonları ve proxy bağlantı çözümleri `ServerService.cs` içindeki `HandleStreamAsync` kısmına aynı mantıkla yansıtılacaktır.
...
- **Çevrimdışı Gönderim ve Üstel Geri Çekilme (Offline Queue & Exponential Backoff):** Firebase gönderimleri doğrudan yapılmaz. Önce yerel SQLite veritabanındaki `PendingFirebasePushes` tablosuna eklenir. `FirebaseQueueService` arka planda `30s * 2^RetryCount` üstel geri çekilme formülü uygulayarak ve ağ değişikliklerini (`NetworkChange.NetworkAddressChanged`) dinleyerek bu kuyruğu asenkron tüketir. Gönderim başarısız olursa maksimum 10 denemeye kadar beklenir, ardından 'failed' durumuna çekilerek veri kaybı önlenir.
- **Okuma İşlemleri (GitHub Raw CDN):** İstemciler sunucu maliyetini sıfırlamak için tüm kanal listesini doğrudan GitHub Raw CDN (`https://raw.githubusercontent.com/bilo1975tr/sm/main`) üzerinden çeker. İstemciler tekilleştirilmiş tek dosya yerine, seçtikleri dil tercihine göre ISO iki haneli dil kodu (`NormalizeLanguageFilename`), bulunamazsa Türkçe dil adı (`GetLegacyLanguageFilename`), o da bulunamazsa `"bilinmiyor"` dil kodundaki CDN dosyalarını çekerek minimum veri tüketimiyle çalışır.
- **Yerel Temizlik ve Çöp Dosya Yasağı (Local Cleanliness & No Scratch Files):** Uygulama dizini altında çalışma anında veya geliştirme esnasında oluşturulan geçici çıktı dosyaları (`test_scrape.js`, `out.txt`, `tr.txt`, `free.txt`), yerel `.m3u` listeleri, test scriptleri ve yedek veritabanı JSON dosyaları (`channels_*.json`, `kanallar_*.m3u`) KESİNLİKLE barındırılmamalıdır. Bu tür çöp dosyalar derhal temizlenmeli ve ana dizinin sade kalması sağlanmalıdır.
