        ...
        public string SaveChannels(List<Channel> channels, string playlistUrl)
        {
            ...
            try
            {
                ...
                using (var connection = new SqliteConnection(ConnectionString))
                {
                    ...
                    var deadLinkHashes = GetAllDeadLinkHashes(); // Belleğe alarak N+1 sorgu sorununu çözüyoruz (Auto Upgrade: Performans Kuralı)
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        foreach (var channel in channels)
                        {
                            ...
                            if (!string.IsNullOrEmpty(channel.Url))
                            {
                                ...
                                {
                                    long hash = GetFnv1aHash(p.Trim());
                                    if (!deadLinkHashes.Contains(hash))
                                    {
